...
module("Integration | Component | input date", function (hooks) {
  ...
  test("when firing the afterParseFail action on the text bla", async function (assert) {
    this.set("afterParseFail", function (inputDateComponent) {
      assert.equal(
        ...
        "bla",
        "The text value is cleared on failed parse."
      );
      assert.equal(
        inputDateComponent.get("date"),
        null,
        "The assigned date is null due to the parse failure."
      );
    });
    ...
    assert.equal(
      ...
      moment(this.get("date")).tz("America/Edmonton").format("ll")
    );
  ...
  test("when firing the afterParseSuccess action on a text valid text value", async function (assert) {
    this.set("afterParseSuccess", function (inputDateComponent) {
      ...
      assert.notEqual(
        inputDateComponent.get("date"),
        null,
        "The parsed date should assigned to the component."
      );
    });
  ...
  test("when firing the afterParseSuccess action on a cleared text field", async function (assert) {
    this.set("afterParseSuccess", function (inputDateComponent) {
      ...
      assert.equal(
        inputDateComponent.get("date"),
        null,
        "The date should now be null because the field was cleared"
      );
    });
  ...
  test("when `past?` set to false requesting `jan` will STILL be in the past", async function (assert) {
    ...
    this.set("date", null);
    await render(
      hbs`{{input-date date=date displayFormat="llll z" past?=false timezone="America/Edmonton"}}`
    );
  ...
  test("when `past?` set to true requesting `jan` will be in the past", async function (assert) {
    ...
    this.set("date", null);
    await render(
      hbs`{{input-date date=date displayFormat="llll z" past?=true timezone="America/Edmonton"}}`
    );
  ...
  test("when `future?` set to false requesting `jan` will be in the past", async function (assert) {
    ...
    this.set("date", null);
    await render(
      hbs`{{input-date date=date displayFormat="llll z" future?=false timezone="America/Edmonton"}}`
    );
  ...
  test("when `future?` set to true requesting `jan` will be in the future", async function (assert) {
    ...
    this.set("date", null);
    await render(
      hbs`{{input-date date=date displayFormat="llll z" future?=true timezone="America/Edmonton"}}`
    );
  ...
  test("when the default timezone is used for Sept 11 2001 at noon", async function (assert) {
    ...
    assert.equal(
      find("input").value.trim(),
      moment(this.get("date")).tz(moment.tz.guess()).format("llll z")
    );
  });
  ...
  test("when assigning a date for Sep 11, 2001 at noon in New York", async function (assert) {
    ...
    await render(
      hbs`{{input-date date=date displayFormat="llll z" timezone="America/New_York"}}`
    );
  ...
  test("when assigning a date for Sep 11, 2001 at noon in Vancouver", async function (assert) {
    this.set(
      "date",
      moment.tz([2001, 8, 11, 12], "America/Vancouver").toDate()
    );
    await render(
      hbs`{{input-date date=date displayFormat="llll z" timezone="America/Vancouver"}}`
    );
